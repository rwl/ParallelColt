 ...
 */
public class DenseFloatSingularValueDecomposition implements java.io.Serializable {
     ...
     */
    public DenseFloatSingularValueDecomposition(FloatMatrix2D Arg) {
        ...
        for (int k = 0; k < max_nct_nrt; k++) {
            if (k < nct) {
                ...
                s[k] = 0;
                for (int i = k; i < m; i++) {
                    s[k] = DenseFloatAlgebra.hypot(s[k], A[i][k]);
                }
                if (s[k] != 0.0) {
                    if (A[k][k] < 0.0) {
                        s[k] = -s[k];
            ...
            if (k < nrt) {
                ...
                e[k] = 0;
                for (int i = k + 1; i < n; i++) {
                    e[k] = DenseFloatAlgebra.hypot(e[k], e[i]);
                }
                if (e[k] != 0.0) {
                    if (e[k + 1] < 0.0) {
                        e[k] = -e[k];
        ...
        while (p > 0) {
            ...
            if (k == p - 2) {
                ...
                for (ks = p - 1; ks >= k; ks--) {
                    ...
                    float t = (ks != p ? Math.abs(e[ks]) : 0.f) + (ks != k + 1 ? Math.abs(e[ks - 1]) : 0.f);
            ...
            case 1: {
                ...
                e[p - 2] = 0.0f;
                for (int j = p - 2; j >= k; j--) {
                    float t = DenseFloatAlgebra.hypot(s[j], f);
                    float cs = s[j] / t;
                    float sn = f / t;
                    s[j] = t;
                    if (j != k) {
            ...
            case 3: {
                ...
                float scale = Math.max(Math.max(Math.max(Math.max(Math.abs(s[p - 1]), Math.abs(s[p - 2])), Math
                ...
                for (int j = k; j < p - 1; j++) {
                    float t = DenseFloatAlgebra.hypot(f, g);
                    float cs = f / t;
                    float sn = g / t;
                    if (j != k) {
                        e[j - 1] = t;
