...
public abstract class DoubleMatrix3D extends AbstractMatrix3D {
	 ...
	 * @see cern.jet.math.DoubleFunctions
	 */
	public double aggregate(final cern.colt.function.DoubleDoubleFunction aggr, final cern.colt.function.DoubleFunction f, final IntArrayList sliceList, final IntArrayList rowList, final IntArrayList columnList) {
		if (size() == 0)
			return Double.NaN;
	 ...
	 * @see cern.jet.math.DoubleFunctions
	 */
	public DoubleMatrix3D assign(final cern.colt.function.DoubleProcedure cond, final cern.colt.function.DoubleFunction f) {
		int np = ConcurrencyUtils.getNumberOfProcessors();
		if ((np > 1) && (slices * rows * columns >= ConcurrencyUtils.getThreadsBeginN_3D())) {
			Future[] futures = new Future[np];
			int k = slices / np;
	 ...
	 * 
	 */
	public DoubleMatrix3D assign(final cern.colt.function.DoubleProcedure cond, final double value) {
		int np = ConcurrencyUtils.getNumberOfProcessors();
		if ((np > 1) && (slices * rows * columns >= ConcurrencyUtils.getThreadsBeginN_3D())) {
			Future[] futures = new Future[np];
			int k = slices / np;
	 ...
	 * @see cern.jet.math.DoubleFunctions
	 */
	public DoubleMatrix3D assign(final DoubleMatrix3D y, final cern.colt.function.DoubleDoubleFunction function, final IntArrayList sliceList, final IntArrayList rowList, final IntArrayList columnList) {
		checkShape(y);
	 ...
	 * 
	 * @see cern.colt.matrix.doublealgo.DoubleFormatter
	 */
	public String toString() {
		return new cern.colt.matrix.doublealgo.DoubleFormatter().toString(this);
	}
	 ...
	 * information, see
	 * {@link cern.colt.matrix.doublealgo.DoubleSorting#sort(DoubleMatrix3D,int,int)}.
	 * For more advanced sorting functionality, see
	 * {@link cern.colt.matrix.doublealgo.DoubleSorting}.
	 * 
	...
	public DoubleMatrix3D viewSorted(int row, int column) {
		return cern.colt.matrix.doublealgo.DoubleSorting.mergeSort.sort(this, row, column);
	}
