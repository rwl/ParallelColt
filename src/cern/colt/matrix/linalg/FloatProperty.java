...
public class FloatProperty extends cern.colt.PersistentObject {
    ...
    public void checkRectangular(FloatMatrix2D A) {
        if (A.rows() < A.columns()) {
            throw new IllegalArgumentException("Matrix must be rectangular: " + cern.colt.matrix.floatalgo.FloatFormatter.shape(A));
        }
    }
    ...
    public void checkSquare(FloatMatrix2D A) {
        if (A.rows() != A.columns())
            throw new IllegalArgumentException("Matrix must be square: " + cern.colt.matrix.floatalgo.FloatFormatter.shape(A));
    }
