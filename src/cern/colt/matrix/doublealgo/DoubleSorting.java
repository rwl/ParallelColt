...
public class DoubleSorting extends cern.colt.PersistentObject {
    ...
    public static void zdemo5(int rows, int columns, boolean print) {
        ...
        if (print) {
            ...
            System.out.println("first part of sorted result = \n" + new cern.colt.matrix.doublealgo.DoubleFormatter("%G").toTitleString(A.viewPart(0, 0, r, columns), rowNames, columnNames, null, null, null, funs));
        }
        ...
        cern.colt.matrix.doublealgo.DoubleMatrix1DComparator fun = new cern.colt.matrix.doublealgo.DoubleMatrix1DComparator() {
            public int compare(DoubleMatrix1D x, DoubleMatrix1D y) {
                double a = cern.colt.matrix.doublealgo.DoubleStatistic.bin(x).median();
                double b = cern.colt.matrix.doublealgo.DoubleStatistic.bin(y).median();
                // double a = x.aggregate(F.plus,F.log);
                // double b = y.aggregate(F.plus,F.log);
                return a < b ? -1 : (a == b) ? 0 : 1;
            }
    ...
    public static void zdemo8(int size) {
        ...
        final cern.jet.math.DoubleFunctions F = cern.jet.math.DoubleFunctions.functions;
        DoubleMatrix1D A = cern.colt.matrix.DoubleFactory1D.dense.random(size);
        System.out.print("now quick sorting... ");
        cern.colt.Timer timer = new cern.colt.Timer().start();
        quickSort.sort(A);
        timer.stop().display();
