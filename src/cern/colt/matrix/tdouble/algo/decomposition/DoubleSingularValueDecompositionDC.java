...
public class DoubleSingularValueDecompositionDC {
    ...
    public DoubleSingularValueDecompositionDC(DoubleMatrix2D Arg, boolean wantUV, boolean wantWholeUV) {
        ...
        m = Arg.rows();
        n = Arg.columns();
        double[] elems = (double[]) Arg.viewDice().copy().elements();
        minmn = Math.min(m, n);
        ...
        if (wantUV = true) {
            if (wantWholeUV) { // JOBZ='A'
                ...
                LAPACK.getInstance().dgesdd("A", m, n, elems, m, s, Ut, m, V, n, work, lwork, iwork, info);
            } else { // JOBZ='S'
                Ut = new double[m * minmn];
                V = new double[minmn * n];
                lwork = 3 * minmn * minmn + Math.max(maxmn, 4 * minmn * minmn + 4 * minmn) + maxmn;
                work = new double[lwork];
                LAPACK.getInstance().dgesdd("S", m, n, elems, m, s, Ut, m, V, minmn, work, lwork, iwork, info);
            }
        } else {// JOBZ='N'
            lwork = 3 * minmn + Math.max(maxmn, 6 * minmn) + maxmn;
            work = new double[lwork];
            LAPACK.getInstance().dgesdd("N", m, n, elems, m, s, null, m, null, n, work, lwork, iwork, info);
        }
    }
