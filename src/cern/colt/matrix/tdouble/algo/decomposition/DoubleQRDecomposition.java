...
public class DoubleQRDecomposition implements java.io.Serializable {
    ...
    public DoubleQRDecomposition(DoubleMatrix2D A) {
        ...
        Rdiag = A.like1D(n);
        // Rdiag = new double[n];
        cern.colt.function.tdouble.DoubleDoubleFunction hypot = DoubleAlgebra.hypotFunction();
        ...
        for (int k = 0; k < n; k++) {
            ...
            if (nrm != 0.0) {
                ...
                if (QR.getQuick(k, k) < 0)
                    nrm = -nrm;
                QRcolumnsPart[k].assign(cern.jet.math.tdouble.DoubleFunctions.div(nrm));
                /*
        ...
        for (int k = n - 1; k >= 0; k--) {
            ...
            for (int j = k; j < n; j++) {
                if (QR.getQuick(k, k) != 0) {
                    ...
                    s = -s / QR.getQuick(k, k);
                    Qcolj.assign(QRcolk, F.plusMultSecond(s));
                }
            }
        }
        return Q;
    ...
    public String toString() {
        ...
        buf.append("\n\npseudo inverse(A) = ");
        try {
            buf.append(String.valueOf(this.solve(cern.colt.matrix.tdouble.DoubleFactory2D.dense.identity(QR.rows()))));
        } catch (IllegalArgumentException exc) {
            buf.append(unknown + exc.getMessage());
        }
