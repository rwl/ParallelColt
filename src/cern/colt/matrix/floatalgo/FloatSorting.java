...
public class FloatSorting extends cern.colt.PersistentObject {
    ...
    public static void zdemo5(int rows, int columns, boolean print) {
        ...
        if (print) {
            ...
            System.out.println("first part of sorted result = \n" + new cern.colt.matrix.floatalgo.FloatFormatter("%G").toTitleString(A.viewPart(0, 0, r, columns), rowNames, columnNames, null, null, null, funs));
        }
        ...
        cern.colt.matrix.floatalgo.FloatMatrix1DComparator fun = new cern.colt.matrix.floatalgo.FloatMatrix1DComparator() {
            public int compare(FloatMatrix1D x, FloatMatrix1D y) {
                float a = cern.colt.matrix.floatalgo.FloatStatistic.bin(x).median();
                float b = cern.colt.matrix.floatalgo.FloatStatistic.bin(y).median();
                // float a = x.aggregate(F.plus,F.log);
                // float b = y.aggregate(F.plus,F.log);
                return a < b ? -1 : (a == b) ? 0 : 1;
            }
    ...
    public static void zdemo8(int size) {
        ...
        final cern.jet.math.FloatFunctions F = cern.jet.math.FloatFunctions.functions;
        FloatMatrix1D A = cern.colt.matrix.FloatFactory1D.dense.random(size);
        System.out.print("now quick sorting... ");
        cern.colt.Timer timer = new cern.colt.Timer().start();
        quickSort.sort(A);
        timer.stop().display();
