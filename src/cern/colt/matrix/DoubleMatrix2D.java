...
public abstract class DoubleMatrix2D extends AbstractMatrix2D {
     ...
     * @see cern.jet.math.DoubleFunctions
     */
    public double aggregate(final cern.colt.function.DoubleDoubleFunction aggr, final cern.colt.function.DoubleFunction f, final IntArrayList rowList, final IntArrayList columnList) {
        if (size() == 0)
            return Double.NaN;
        ...
        if ((np > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            ...
            for (int j = 0; j < np; j++) {
                ...
                futures[j] = ConcurrencyUtils.threadPool.submit(new Callable<Double>() {
                    ...
                    public Double call() throws Exception {
                        double a = f.apply(getQuick(rowElements[startidx], columnElements[startidx]));
                        double elem;
     ...
     * @see cern.jet.math.DoubleFunctions
     */
    public DoubleMatrix2D assign(final DoubleMatrix2D y, final cern.colt.function.DoubleDoubleFunction function, IntArrayList rowList, IntArrayList columnList) {
        checkShape(y);
        final int size = rowList.size();
        final int[] rowElements = rowList.elements();
        final int[] columnElements = columnList.elements();
     ...
     * {@link cern.colt.matrix.doublealgo.DoubleSorting#sort(DoubleMatrix2D,int)}. For
    ...
    public DoubleMatrix2D viewSorted(int column) {
        return cern.colt.matrix.doublealgo.DoubleSorting.mergeSort.sort(this, column);
    }
