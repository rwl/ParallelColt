...
public abstract class FloatMatrix3D extends AbstractMatrix3D {
		...
		if ((np > 1) && (slices * rows * columns >= ConcurrencyUtils.getThreadsBeginN_3D())) {
			...
			for (int j = 0; j < np; j++) {
				...
				futures[j] = ConcurrencyUtils.threadPool.submit(new Callable<Float>() {
					...
					public Float call() throws Exception {
						float elem = getQuick(startslice, 0, 0);
						float a = 0;
	 ...
	 * @see cern.jet.math.FloatFunctions
	 */
	public float aggregate(final cern.colt.function.FloatFloatFunction aggr, final cern.colt.function.FloatFunction f, final IntArrayList sliceList, final IntArrayList rowList, final IntArrayList columnList) {
		if (size() == 0)
			return Float.NaN;
		...
		if ((np > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_3D())) {
			...
			for (int j = 0; j < np; j++) {
				...
				futures[j] = ConcurrencyUtils.threadPool.submit(new Callable<Float>() {
					...
					public Float call() throws Exception {
						float a = f.apply(getQuick(sliceElements[startidx], rowElements[startidx], columnElements[startidx]));
						float elem;
	 ...
	 * @see cern.jet.math.FloatFunctions
	 */
	public FloatMatrix3D assign(final cern.colt.function.FloatProcedure cond, final cern.colt.function.FloatFunction f) {
		int np = ConcurrencyUtils.getNumberOfProcessors();
		if ((np > 1) && (slices * rows * columns >= ConcurrencyUtils.getThreadsBeginN_3D())) {
			Future[] futures = new Future[np];
			int k = slices / np;
	 ...
	 * 
	 */
	public FloatMatrix3D assign(final cern.colt.function.FloatProcedure cond, final float value) {
		int np = ConcurrencyUtils.getNumberOfProcessors();
		if ((np > 1) && (slices * rows * columns >= ConcurrencyUtils.getThreadsBeginN_3D())) {
			Future[] futures = new Future[np];
			int k = slices / np;
	 ...
	 * @see cern.jet.math.FloatFunctions
	 */
	public FloatMatrix3D assign(final FloatMatrix3D y, final cern.colt.function.FloatFloatFunction function, final IntArrayList sliceList, final IntArrayList rowList, final IntArrayList columnList) {
		checkShape(y);
	 ...
	 * 
	 * @see cern.colt.matrix.floatalgo.FloatFormatter
	 */
	public String toString() {
		return new cern.colt.matrix.floatalgo.FloatFormatter().toString(this);
	}
	 ...
	 * information, see
	 * {@link cern.colt.matrix.floatalgo.FloatSorting#sort(FloatMatrix3D,int,int)}.
	 * For more advanced sorting functionality, see
	 * {@link cern.colt.matrix.floatalgo.FloatSorting}.
	 * 
	...
	public FloatMatrix3D viewSorted(int row, int column) {
		return cern.colt.matrix.floatalgo.FloatSorting.mergeSort.sort(this, row, column);
	}
