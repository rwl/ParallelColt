 ...
 * size of data must be a power-of-two number. This is a parallel implementation
...
public class FloatDCT_1D {
     ...
     *            if true then scaling is performed
     */
    public void forward(float[] a, int offa, boolean scale) {
        if (n == 1)
            return;
        int j, nw, nc;
        float xr;
    ...
    private void scale(final float[] a, final int offa) {
        int np = ConcurrencyUtils.getNumberOfProcessors();
        final float m = (float) Math.sqrt(2.0 / n);
