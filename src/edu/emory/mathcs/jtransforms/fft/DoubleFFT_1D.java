...
public class DoubleFFT_1D {
    ...
    public DoubleFFT_1D(int n) {
        ...
        if (!ConcurrencyUtils.isPowerOf2(n)) {
            if (getReminder(n, factors) >= 211) {
                ...
                n_b = ConcurrencyUtils.nextPow2(n * 2 - 1);
                bk1 = new double[2 * n_b];
                bk2 = new double[2 * n_b];
                this.ip = new int[2 + (int) Math.ceil(2 + (1 << (int) (Math.log(n_b + 0.5) / Math.log(2)) / 2))];
                this.w = new double[n_b];
                int twon = 2 * n_b;
                nw = ip[0];
                if (twon > (nw << 2)) {
                    nw = twon >> 2;
    ...
    private void bluesteini() {
        ...
        for (int i = 2; i < 2 * n; i += 2) {
            ...
            bk2[2 * n_b - i] = bk2[i];
            bk2[2 * n_b - i + 1] = bk2[i + 1];
        }
        cftbsub(2 * n_b, bk2, 0, ip, nw, w);
    }
    ...
    private void bluestein_complex(final double[] a, final int offa, final int isign) {
        final double[] ak = new double[2 * n_b];
        int np = ConcurrencyUtils.getNumberOfProcessors();
        if ((np > 1) && (n > ConcurrencyUtils.getThreadsBeginN_1D_FFT_2Threads())) {
            np = 2;
            if ((np >= 4) && (n > ConcurrencyUtils.getThreadsBeginN_1D_FFT_4Threads())) {
                np = 4;
            }
            Future[] futures = new Future[np];
            int k = n / np;
    ...
    // isign is +1 for backward and -1 for forward transforms
    // ----------------------------------------------------------------------*/
    // void passf6(final int ido, final int l1, final double a[], final int
    // offa, final double ch[], final int offch, final int offset, final int
    // isign)
    // /*isign==-1 for forward transform and+1 for backward transform*/
    // {
    // double tau = isign * Math.sqrt(3.0) / 2.0;
    // int i, k;
    // double ci2, ci3, ci4, ci5, di3, di4, di5, di2, cr2, cr3, cr5, cr4, ti1,
    // tr1, ti2, ti3, ti4, ti5, dr3, dr4, dr5, dr2, tr2, tr3, tr4, tr5;
    ...
    // double a6i = a[idx6 - 1];
    //				
    // double w1r = wtable[idx12];
    // double w1i = isign * wtable[idx12 + 1];
    // double w2r = wtable[idx13];
    // double w2i = isign * wtable[idx13 + 1];
    // double w3r = wtable[idx14];
    // double w3i = isign * wtable[idx14 + 1];
    // double w4r = wtable[idx15];
    // double w4i = isign * wtable[idx15 + 1];
    // double w5r = wtable[idx16];
    // double w5i = isign * wtable[idx16 + 1];
    //				
    // ti5 = a1r - a5r;
    // ti2 = a1r + a5r;
    // ti4 = a2r - a4r;
    ...
    void passfg_new(final int nac[], final int ido, final int ip, final int l1, final int idl1, final double in[], final int in_off, final double out[], final int out_off, final int offset, final int isign) {
