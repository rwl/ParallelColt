 ...
 * check out {@link cern.jet.stat.tdouble.DoubleDescriptive} in combination with methods
...
public class DynamicDoubleBin1D extends QuantileDoubleBin1D {
     ...
     */
    public synchronized double aggregate(cern.colt.function.tdouble.DoubleDoubleFunction aggr, cern.colt.function.tdouble.DoubleFunction f) {
        int s = size();
        if (s == 0)
            return Double.NaN;
     ...
     */
    public synchronized void sample(int n, boolean withReplacement, DoubleRandomEngine randomGenerator, cern.colt.buffer.tdouble.DoubleBuffer buffer) {
        if (randomGenerator == null)
            randomGenerator = cern.jet.random.tdouble.DoubleUniform.makeDefaultGenerator();
        buffer.clear();

        if (!withReplacement) { // without
            if (n > size())
                throw new IllegalArgumentException("n must be less than or equal to size()");
            cern.jet.random.tdouble.sampling.DoubleRandomSamplingAssistant sampler = new cern.jet.random.tdouble.sampling.DoubleRandomSamplingAssistant(n, size(), randomGenerator);
            for (int i = n; --i >= 0;) {
     ...
     */
    public synchronized DynamicDoubleBin1D sampleBootstrap(DynamicDoubleBin1D other, int resamples, cern.jet.random.tdouble.engine.DoubleRandomEngine randomGenerator, DoubleBinBinFunction1D function) {
        if (randomGenerator == null)
            randomGenerator = cern.jet.random.tdouble.DoubleUniform.makeDefaultGenerator();
