...
public class BenchmarkDenseFloatMatrix1D {
	...
	public static void main(String[] args) {
		org.junit.runner.JUnitCore.main("cern.colt.matrix.impl.BenchmarkDenseFloatMatrix1D");
	}
	...
	public void testReshapeIntInt() {
		...
		for (int i = 0; i < BenchmarkMatrixKernel.NTHREADS.length; i++) {
			...
			// warm-up
			FloatMatrix2D B = Av.reshape(rows, cols);
			for (int j = 0; j < BenchmarkMatrixKernel.NITERS; j++) {
				t.reset().start();
	...
	public void testReshapeIntIntInt() {
		...
		for (int i = 0; i < BenchmarkMatrixKernel.NTHREADS.length; i++) {
			...
			// warm-up
			FloatMatrix3D B = Av.reshape(slices, rows, cols);
			for (int j = 0; j < BenchmarkMatrixKernel.NITERS; j++) {
				t.reset().start();
