...
//    private static final int[] sizes2D = new int[] { 2000, 2048, 4000, 4096, 8000, 8192 };
    ...
    public static void benchmarkFft2() {
        ...
        for (int i = 0; i < sizes2D.length; i++) {
            ...
            System.out.println("Average execution time for single precision getFft2() of size " + sizes2D[i] + " x " + sizes2D[i]
                    + " : " + times[i] + " ms");
    ...
    public static void benchmarkFft3() {
        ...
        for (int i = 0; i < sizes3D.length; i++) {
            DenseFloatMatrix3D A = (DenseFloatMatrix3D) FloatFactory3D.dense
            ...
            double p = (2.5 * N * log2(N)) / (t.nanos() / niters);
            System.out.println("Average execution time for single precision getFft3() of size " + sizes3D[i] + " x " + sizes3D[i]
                    + " x " + sizes3D[i] + " : " + times[i] + " ms");
            System.out.println("Performance of single precision getFft3() of size " + sizes3D[i] + " x " + sizes3D[i] + " x "
